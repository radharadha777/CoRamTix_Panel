...
class UpdateChecker extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            $response = Http::get('https://coramtix.dev/api/v2/get-version');
            if ($response->failed()) {
            ...
            if (!$latestVersion) {
                $this->error('Server sent invalid response. Reach out to CoRamTix support team.');
                return Command::FAILURE;
            }
            ...
            if ($latestVersion === $currentVersion) {
                ...
                $this->info('You\'re running pre-release of CoRamTix Panel.');
            }
        } catch (\Exception $e) {
