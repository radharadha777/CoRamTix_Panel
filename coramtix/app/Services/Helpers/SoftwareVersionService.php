...
class SoftwareVersionService
{
    ...
    {
        return Arr::get(self::$result, 'discord') ?? 'https://coramtix.dev/discord';
    }
    ...
    {
        return $this->cache->remember(self::VERSION_CACHE_KEY, CarbonImmutable::now()->addMinutes(config('coramtix.cdn.cache_time', 60)), function () {
            try {
                $response = $this->client->request('GET', config('coramtix.cdn.url'));
