...
import Card from '@/coramtix/ui/Card';
import Title from '@/coramtix/ui/Title';
import tw from 'twin.macro';
import styled from 'styled-components/macro';
import { LogoContainer } from '@/coramtix/ui/LogoContainer';
...
export default () => {
    const logo = useStoreState((state: ApplicationStore) => state.coramtix.data!.logo);
    const maintenance = useStoreState((state) => state.coramtix.data!.maintenance);
    ...
    return (
        <PageContentBlock className='flex flex-col h-full' title={'Under Maintenance'} showFlashKey={'dashboard'}>
            <Container>
                <CardContainer>
                    <LogoContainer>
                        <img src={logo} alt='coramtix' css={tw`h-[3rem]`} />
                    </LogoContainer>
                    <Card>
